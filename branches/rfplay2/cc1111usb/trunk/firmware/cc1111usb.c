...
xdata u8 appstatus;
xdata u8 lastCode[2];
xdata DMA_CONFIG dma1_config;
xdata DMA_CONFIG dma2_config;
xdata DMA_CONFIG dma3_config;
xdata DMA_CONFIG dma4_config;
...
void dma_init(void)
{
    // usb IN dma config - use Static Length
    dma1_config.SRCADDR     = 0;        // this needs to be set before each use
    dma1_config.DESTADDR    = USBF0;
    dma1_config.VLEN        = 0;        // static length.  set this at time of transfer
    dma1_config.LEN         = 0;        // static length.  set this at time of transfer
    dma1_config.WORDSIZE    = 0;
    dma1_config.TMODE       = 3;        // repeated block xfer
    dma1_config.TRIG        = 0;        // manual trigger
    dma1_config.SRCINC      = 1;
    dma1_config.DESTINC     = 0;
    dma1_config.IRQMASK     = 0;
    dma1_config.M8          = 0;
    dma1_config.PRIORITY    = 2;        // high priority

    // usb OUT dma config - use Static Length
    dma2_config.SRCADDR     = USBF0;        // this needs to be set before each use
    dma2_config.DESTADDR    = (u16)usb_ep0_OUTbuf;
    dma2_config.VLEN        = 0;        // static length.  set this at time of transfer
    dma2_config.LEN         = 0;        // static length.  set this at time of transfer
    dma2_config.WORDSIZE    = 0;
    dma2_config.TMODE       = 3;        // repeated block xfer
    dma2_config.TRIG        = 0;        // manual trigger
    dma2_config.SRCINC      = 0;
    dma2_config.DESTINC     = 1;
    dma2_config.IRQMASK     = 0;
    dma2_config.M8          = 0;
    dma2_config.PRIORITY    = 2;        // high priority


    DMA1CFGH                = (u16)(&dma1_config)>>8;
    DMA1CFGL                = ((u16)&dma1_config)&0xff;
    //blink_binary_baby_lsb(DMA1CFGL, 8);
    //blink_binary_baby_lsb((u16)&dma1_config, 8);
    DMAARM = 2;
}
...
void usb_arm_ep0IN(){
    ...
    dma1_config.LEN = tlen;
    dma1_config.SRCADDR = (u16)ep0iobuf.INbuf;
    DMAARM = 2;
    sleepMillis(1);
    DMAREQ = 2;
    while (!(DMAIRQ & 2))
        blink(100,200);                    // wait for transfer to complete
    /////
    /////
