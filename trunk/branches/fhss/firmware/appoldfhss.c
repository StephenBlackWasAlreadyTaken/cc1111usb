...
/**************************** MAC LAYER *****************************/
MAC_begin_hopping(u16 T1_offset)
{
...
MAC_set_chanidx(u16 chanidx)
{
    PHY_set_channel( g_hopping_pattern[ chanidx ] );
}

MAC_tx(u8 len, u8* message)
{
    //
}

MAC_rx_handle(u8 len, u8* message)
{
    //
}
